...
import { X, Upload, AlertCircle } from "lucide-react"; // or your icon library
...
const BusinessSealModal = ({ isOpen, onClose, business, onUpdateSuccess }) => {
  ...
  const handleSubmit = () => {
    ...
    _postApi(
      `/account/update-seal/${business.id}`,
      {
        seal,
      ...
      (resp) => {
        if (resp.success) {
          ...
          toast.success("Business seal updated successfully!");
          onClose();
        } else {
          toast.error(resp.message || "Failed to update seal.");
        }
        setLoading(false);
      },
      (err) => {
        console.error("Seal uploaStaff Signatured error:", err);
        setLoading(false);
        toast.error("Network error. Could not upload seal.");
      }
    );
  };
          ...
          <div className="p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-gray-900">
                {business.seal ? "Update" : "Add"} Business Seal
              </h2>
              <button
                onClick={onClose}
...
export default BusinessSealModal;
