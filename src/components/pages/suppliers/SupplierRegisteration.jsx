...
import { _postApi, _putApi, _fetchApi } from "@/redux/actions/api";
...
const SupplierRegisteration = ({
  ...
  useEffect(() => {
    if (selectedSupplier) {
      const openingBalance = selectedSupplier.opening_balance || null;
      const existingName =
      ...
      setForm({
        ...
        work_phone: selectedSupplier.phone || "",
  ...
  const selectedPayableAccount = chartOfAccount.find(
    (account) => account.head === form.payable_code,
  );
  const selectedPayableAccuralAccount = chartOfAccount.find(
    (account) => account.head === form.payable_accural_code,
  );
  ...
  const validateForm = () => {
    ...
    if (!String(form.payable_code || "").trim()) {
      newErrors.payable_code = "Accounts payable is required";
    }
    ...
    const parsedOpeningBalance = form.opening_balance
      ...
      : 0;
    if (!selectedSupplier && parsedOpeningBalance !== 0) {
      if (!form.obdate) {
        newErrors.obdate =
    ...
    else if (newErrors.payable_code || newErrors.payable_accural_code)
      setActiveTab("other");
  ...
  const handleSubmit = (e) => {
    ...
    try {
      ...
      if (selectedSupplier) {
        const updatePayload = {
          ...basePayload,
          payable_code: form.payable_code,
          payable_accural_code: form.payable_accural_code,
          branch_id: form.branch_id || null,
        };
        _putApi(
          `/api/suppliers/${facilityId}/${selectedSupplier.supplier_number}`,
          updatePayload,
          (res) => {
            setLoading(false);
            if (!res.success) {
            ...
            toast.success(`Vendor ${displayName} updated successfully`);
            success_callback();
          },
          (err) => {
            setLoading(false);
        ...
        const payload = {
          ...
          facilityId,
          payable_code: form.payable_code,
          advance_code: form.payable_accural_code,
          branch_id: form.branch_id || null,
          created_by: user.id,
        ...
        _postApi(
          ...
          (res) => {
            ...
            const supplierNo = res.data?.supplier_number || "N/A";
            toast.success(
          ...
          (err) => {
            setLoading(false);
            console.error(err);
            toast.error("An error occurred while saving vendor!");
          },
        );
      }
    } catch (err) {
      ...
      >
        <SheetHeader className="shrink-0 space-y-1 border-b border-slate-200 bg-[var(--aa-navy)] px-5 py-4 text-left">
          <SheetTitle className="text-lg font-semibold text-white">
            {selectedSupplier ? "Edit Vendor" : "New Vendor"}
          </SheetTitle>
          <SheetDescription className="text-sm text-white/70">
            {selectedSupplier
              ? "Update vendor details"
              : "Add a supplier / vendor to your purchase list"}
          </SheetDescription>
        </SheetHeader>
          ...
          <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-4">
            ...
            >
              ...
              <TabsContent value="other" className="mt-4 space-y-4">
                ...
                <div>
                  ...
                  <TypeaheadCustom
                    ...
                    onChange={(selectedItems) => {
                      setForm((prev) => ({
                        ...prev,
                        payable_code: selectedItems[0]?.head || "",
                      }));
                    }}
                    fixed
                    flip
                    selected={
                      selectedPayableAccount ? [selectedPayableAccount] : []
                    }
                  />
                  {errors.payable_code && (
                ...
                <div>
                  <ShadcnLabel className={labelClass}>
                    Advance to Payables <span className="text-red-500">*</span>
                  </ShadcnLabel>
                  <TypeaheadCustom
                    options={chartOfAccount}
                    placeholder="Select an account"
                    ...
                    onChange={(selectedItems) => {
                      setForm((prev) => ({
                        ...prev,
                        payable_accural_code: selectedItems[0]?.head || "",
                      }));
                    }}
                    fixed
                    flip
                    selected={
                      selectedPayableAccuralAccount
                        ? [selectedPayableAccuralAccount]
                        : []
                    }
                  />
                  {errors.payable_accural_code && (
                    <p className="mt-1 text-xs text-red-500">
                      {errors.payable_accural_code}
                    </p>
                  )}
              ...
              <TabsContent value="address" className="mt-4 space-y-5">
                ...
                <p className="text-[11px] text-slate-500">
                  Note: Billing address is saved with the vendor record.
                </p>
              </TabsContent>
...
SupplierRegisteration.propTypes = {
