/**
 * Supplier-level AP aging buckets.
 */
export default function PayableAgingSummaryTable({ rows, totals, formatCurrency }) {
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-collapse text-sm">
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.supplierNo} className="border-b border-gray-200">
              <td className="px-3 py-2 text-gray-900">
                <span className="font-medium block">{r.supplierName}</span>
                <span className="text-gray-500 text-xs">{r.supplierNo}</span>
              </td>
