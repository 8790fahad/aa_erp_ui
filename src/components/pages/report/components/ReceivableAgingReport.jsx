...
import { aggregateCustomerAgingFromInvoices } from "../utils/receivableAgingFromInvoices";
...
export default function ReceivableAgingReport() {
    ...
    _fetchApi(
      ...
      (response) => {
        ...
        if (response?.success && Array.isArray(response.results)) {
          ...
          setInvoices([]);
          toast.error(response?.message || "Unable to load outstanding invoices");
        }
      },
      (err) => {
        setLoading(false);
        ...
        toast.error("Unable to load outstanding invoices");
      },
    );
  ...
  }, [facilityId, fetchAllOutstandingInvoices]);
    ...
    _fetchApi(
      ...
      (response) => {
        if (response?.success) {
          ...
          const map = {};
          list.forEach((c) => {
            const key = normalizeCustomerNo(c.customerNo || c.id);
            if (!key) return;
            map[key] = c.fullname || c.name || c.customerNo || key;
          });
  ...
  const enrichedInvoices = useMemo(
    () =>
      invoices.map((inv) => {
        if (inv.customer_name) return inv;
        const fallbackName =
          customerNameMap[normalizeCustomerNo(inv.ref_number)] || null;
        return fallbackName
          ? { ...inv, customer_name: fallbackName }
          : inv;
      }),
  ...
  const { rows, totals } = useMemo(
    () => aggregateCustomerAgingFromInvoices(enrichedInvoices, asAtDate),
    [enrichedInvoices, asAtDate],
  );
  ...
  const handleExportExcel = useCallback(async () => {
    ...
    try {
      ...
      r++;
      ws.mergeCells(r, 1, r, 6);
      ws.getCell(r, 1).value = "Accounts Receivable Aging Report";
      ...
      for (const row of rows) {
        ws.getCell(r, 1).value = `${row.customerName} (${row.customerNo})`;
      ...
      a.href = url;
      a.download = `ar-aging-${asAtDate}.xlsx`;
      a.click();
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    try {
      ...
      pdf.save(`ar-aging-${asAtDate}.pdf`);
            ...
            >
              ...
              {rows.length === 0 ? (
                <div className="px-3 py-12 text-center text-sm text-gray-500">
                  No outstanding receivables for this facility, or all invoices
                  are fully paid.
                </div>
              ) : (
