...
/**
 * Zoho-style Record Payment — label-left rows, customer chip,
 * itemized unpaid-invoices table, totals summary box.
 */
export default function RecordPaymentForm() {
  ...
  /** When true, changing Amount Received should not wipe manual invoice allocations. */
  const skipAutoAllocateRef = useRef(false);
  /** User edited payment cells — don't re-FIFO-allocate on invoice list refresh. */
  ...
  const generatePaymentNumber = useCallback(() => {
    ...
    _fetchApi(
      `/get-and-update/cus_dep/${facilityId}`,
      (resp) => {
        if (resp?.success) {
          const n = `${resp.results}`.padStart(5, "0");
          setPaymentNumber(`Ref-${n}`);
        }
      },
      () => setPaymentNumber(`Ref-${Date.now().toString().slice(-5)}`),
    );
  }, [facilityId]);
  ...
  const fetchOutstanding = useCallback(
    (customerNo) => {
      ...
      _fetchApi(
        `/api/v1/get-outstanding-invoices?customerNo=${customerNo}&facilityId=${facilityId}&userId=${
          user?.id || ""
        }`,
        (response) => {
  ...
  const allocateFromReceived = useCallback((totalStr) => {
  ...
  // Auto-apply Amount Received → invoices (FIFO). Skipped after manual payment edits
  // so under-applying can leave a visible Amount in Excess.
  useEffect(() => {
    if (outstandingInvoices.length === 0) return;
    ...
    allocateFromReceived(amountReceived);
  }, [amountReceived, outstandingInvoices, allocateFromReceived]);
  ...
  const receivedNum = parseFloat(parseNumberFromFormatted(amountReceived)) || 0;
  const excess = Math.max(0, +(receivedNum - allocatedSum).toFixed(2));

  const syncAmountReceivedUp = useCallback((nextAllocated) => {
    const received = parseFloat(parseNumberFromFormatted(amountReceived)) || 0;
    if (nextAllocated > received + 0.001) {
      skipAutoAllocateRef.current = true;
      setAmountReceived(formatNumberWithCommas(nextAllocated.toFixed(2)));
    }
  }, [amountReceived]);
  ...
  const setInvoicePayment = (inv, raw) => {
    ...
    setInvoicePayments((prev) => {
      ...
      for (const row of outstandingInvoices) {
        ...
        sum +=
          parseFloat(parseNumberFromFormatted(next[key] || "")) || 0;
      }
      syncAmountReceivedUp(sum);
      return next;
    });
  };
  ...
  const handleAmountReceivedChange = (raw) => {
    manualAllocationRef.current = false;
    setAmountReceived(formatNumberWithCommas(raw));
  ...
  const validate = () => {
    ...
    const amt = parseFloat(parseNumberFromFormatted(amountReceived)) || 0;
    if (amt <= 0) next.amount = "Amount received is required";
    ...
    if (modeOfPayment === "cash" && !accountHead?.head) {
      next.depositTo = "Deposit To is required";
    }
    ...
    if (
      ...
      toast.error("Receivable accounts are not configured for this business");
      return false;
    }
    if (allocatedSum > amt + 0.01) {
      toast.error(
        `Applied payments (${formatNumber1(allocatedSum)}) cannot exceed amount received (${formatNumber1(amt)})`,
      );
      return false;
    }
    setErrors(next);
  ...
  const handleSave = () => {
    ...
    const amountPaidNum =
      parseFloat(parseNumberFromFormatted(amountReceived)) || 0;
    const invoicesWithPayments = outstandingInvoices
      .map((invoice) => {
    ...
    const bankChargesNum = parseFloat(parseNumberFromFormatted(bankCharges));
    if (bankChargesNum > 0) {
      notesParts.push(`Bank charges: ${formatNumber1(bankChargesNum)}`);
    }
    ...
    const narration =
      ...
      `Payment received${paymentNumber ? ` ${paymentNumber}` : ""}`;
      ...
      (res) => {
        ...
        if (res?.success) {
          const referenceNumber =
            ...
            "";
          const advanceAmt = parseFloat(res.data?.advanceAmount) || 0;
          const appliedAmt = parseFloat(res.data?.appliedToReceivable) || 0;
          let msg =
            res.message ||
            `Payment recorded${referenceNumber ? ` (${referenceNumber})` : ""}`;
          if (advanceAmt > 0 && appliedAmt > 0) {
            msg = `Applied ${formatNumber1(appliedAmt)}; advance ${formatNumber1(advanceAmt)}${referenceNumber ? ` (${referenceNumber})` : ""}`;
          } else if (advanceAmt > 0) {
            msg = `Advance ${formatNumber1(advanceAmt)} recorded${referenceNumber ? ` (${referenceNumber})` : ""}`;
          } else if (referenceNumber) {
            msg = `Payment recorded (${referenceNumber})`;
          }
          toast.success(msg);
          navigate("/app/payments/receive-payment", { replace: true });
        } else {
          toast.error(res?.error || res?.message || "Failed to save payment");
        }
      },
      (error) => {
        setSaving(false);
      ...
      <div className="mx-auto max-w-8xl">
        ...
        {showBanner && (
          <div className="mb-3 flex items-start gap-2 rounded border border-blue-100 bg-blue-50 px-3 py-2 text-sm text-blue-900">
            ...
            <p className="flex-1">
              Apply this payment directly to unpaid invoices below to keep
              your customer&apos;s outstanding balance up to date.
            </p>
            <button
              type="button"
        ...
        <Row label="Amount Received" required error={errors.amount}>
          ...
          <p className="mt-1 text-[11px] text-gray-500">
            ...
            amount as customer advance. Any Amount in Excess also becomes
            advance.
          </p>
        </Row>

        <Row label="Bank Charges (if any)">
          <input
            type="text"
            inputMode="decimal"
            value={bankCharges}
            onChange={(e) =>
              setBankCharges(formatNumberWithCommas(e.target.value))
            }
            disabled={saving}
            placeholder="0.00"
        ...
        <Row label="Deposit To" required error={errors.depositTo}>
          {modeOfPayment === "cash" ? (
            <Typeahead
              ref={cashTypeaheadRef}
              id="record-payment-cash-head"
              labelKey={(option) =>
              ...
              onChange={(selectedItems) => {
                ...
                setErrors((prev) => ({ ...prev, depositTo: undefined }));
              }}
              selected={
                accountHead?.head
                    ...
                    <tr key={k} className="hover:bg-slate-50/60">
                      ...
                      <td className="px-3 py-2 text-right">
                        <input
                          ...
                          onChange={(e) => setInvoicePayment(inv, e.target.value)}
                          disabled={saving}
                          placeholder="0.00"
        ...
        >
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:divide-x sm:divide-gray-200">
            <div className="flex min-w-0 flex-col gap-1 px-2">
              <span className="text-[12px] text-gray-600">Amount Received :</span>
          ...
          {excess > 0 ? (
            <p className="mt-2 px-2 text-[11px] font-medium text-red-600">
              Excess {currency} {formatNumber1(excess)} will be kept as customer
              credit / advance.
            </p>
          ) : null}
        </div>

        {/* Notes + Attachments — one row */}
